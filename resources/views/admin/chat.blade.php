            ...
            window.Echo.join('chat')
                .here((users) => {
                    users.forEach((user, index) => {
                        ...
                        //console.log(user.id)
                        element.setAttribute('onclick', `sendMessagePrivate("${user.id}")`)
...
<script>
    function sendMessagePrivate($id) {
        window.axios.post('/chat/message/' + $id)
    }
</script>
<script type="module">
    const messagesElement2 = document.getElementById('messages')
    Echo.private('chat.private.{{auth()->user()->student->id}}')
        .listen('PrivateMessageSent', (e) => {
            const element = document.getElementById('li')
            console.log(e);
            element.innerText = e.message
            element.classList.add('text-success')
            messagesElement2.appendChild(element)
        })
</script>
